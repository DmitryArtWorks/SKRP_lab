...
for angleViz = 1:NumDirs
    ...
    for j = C
        ...
        V_1z(j,1) = V_1(j,1)*sin(psi_1(j,1));                          % проекци€ скорости на ось z [м/с]
        V_1x(j,1) = V_1(j,1)*cos(psi_1(j,1));                          % проекци€ скорости на ось x [м/с]
    
        a_1(j,1) = sqrt(j_1prd(j,1)^2 + j_1ppr(j,1)^2);                % ускорение [м/с^2]
    
        psi_a1(j,1)     = psi_1(j,1) + atan2(j_1ppr(j,1),j_1prd(j,1)); % направление ускорени€ [рад]
        psi_a1(j,1)     = ANGLE_CORRECT( psi_a1(j,1) );
        psi_a1_deg(j,1) = psi_a1(j,1) *180/pi;                         % направление ускорени€ [град]
    
        a_1z(j,1) = a_1(j,1)*sin(psi_a1(j,1));                         % проекци€ ускорени€ на ось z [м/с^2]
        a_1x(j,1) = a_1(j,1)*cos(psi_a1(j,1));                         % проекци€ ускорени€ на ось x [м/с^2]
    ...
    switch sw_cel
        case 0
            ...
            V_2z(1,1) = V_2(1,1)*sin(psi_2(1,1));                          % проекци€ скорости на ось z [м/с]
            V_2x(1,1) = V_2(1,1)*cos(psi_2(1,1));                          % проекци€ скорости на ось x [м/с]
    
            a_2(1,1) = sqrt(j_2prd(1,1)^2 + j_2ppr(1,1)^2);                % ускорение [м/с^2]
    
            psi_a2(1,1)     = psi_2(1,1) + atan2(j_2ppr(1,1),j_2prd(1,1)); % направление ускорени€ [рад]
            psi_a2(1,1)     = ANGLE_CORRECT( psi_a2(1,1) );
            psi_a2_deg(1,1) = psi_a2(1,1) *180/pi;                         % направление ускорени€ [град]
    
            a_2z(1,1) = a_2(1,1)*sin(psi_a2(1,1));                         % проекци€ ускорени€ на ось z [м/с^2]
            a_2x(1,1) = a_2(1,1)*cos(psi_a2(1,1));                         % проекци€ ускорени€ на ось x [м/с^2]
    
        case 1
    
            for j = C
    
                V_2z(j,1) = V_2(j,1)*sin(psi_2(j,1));                          % проекци€ скорости на ось z [м/с]
                V_2x(j,1) = V_2(j,1)*cos(psi_2(j,1));                          % проекци€ скорости на ось x [м/с]
    
                a_2(j,1) = sqrt(j_2prd(j,1)^2 + j_2ppr(j,1)^2);                % ускорение [м/с^2]
    
                psi_a2(j,1)     = psi_2(j,1) + atan2(j_2ppr(j,1),j_2prd(j,1)); % направление ускорени€ [рад]
                psi_a2(j,1)     = ANGLE_CORRECT( psi_a2(j,1) );
                psi_a2_deg(j,1) = psi_a2(j,1) *180/pi;                         % направление ускорени€ [град]
    
                a_2z(j,1) = a_2(j,1)*sin(psi_a2(j,1));                         % проекци€ ускорени€ на ось z [м/с^2]
                a_2x(j,1) = a_2(j,1)*cos(psi_a2(j,1));                         % проекци€ ускорени€ на ось x [м/с^2]
